...
import { useGetTaskQuery, useUpdateTaskMutation } from "@/redux/services/tasks";
...
function UpdateTaskPage({ params }: Props) {
  const { task } = useAppSelector((state) => state.tasks);
  const dispatch = useAppDispatch();
  const [updateTask] = useUpdateTaskMutation();
  const router = useRouter();
  const { id } = params;
  const { isSuccess, data } = useGetTaskQuery(id);
    ...
    <section className="min-h-[calc(100vh-76px)] flex justify-center items-center">
      ...
      >
        ...
        <div className="flex justify-between items-center">
          <label htmlFor="completed" className="text-xl sm:text-2xl">
            It's completed?
          </label>
          <input
            name="completed"
        ...
        <button
          type="submit"
          className="bg-green-700 hover:bg-green-600 transition-colors ease-in py-2 px-4 text-xl sm:text-2xl text-gray-50 rounded-full"
        >
          Update Task
        </button>
      </form>
